...
//stop/play 버튼 클릭시 타이머 동작/중지
  ...
  $('.product .btn_box .lr_btn').click(function(event){
    event.preventDefault();
    clearInterval(timeonoff); 
    var clk_ind=$(this).index('.lr_btn');
    console.log(clk_ind);
    if(clk_ind==0){
        clk_cnt--;
        if(clk_cnt<1){clk_cnt=1;}
        console.log(111);
        $('.product ul').css({left:-100*(clk_cnt-1)+'%'});
    } else if(clk_ind==1){
        clk_cnt++;
        if(clk_cnt>4){clk_cnt=4;}
        $('.product ul').css({left:-100*(clk_cnt-1)+'%'});
    }
    // clk_cnt=1 위치 left:0
    // clk_cnt=2 위치 left:-100%;
    // clk_cnt=3 위치 left:-200%
    // clk_cnt=4 위치 left:-300%;
});
});
